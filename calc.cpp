...
constexpr char print = ';';    // Q)3 Drill)6; changing ; to = not possible bc used under char
	  ...
	  if (t.kind != ')') error ("')'expected");
		return sqrt(d);   }
		 case power: {
			...
			if(t.kind != ',') error("',' expected");
			t=ts.get();
			if(t.kind != number) error("second argument has to be positive integer");
			int i = int(t.value);
		  if(i != t.value) error("second argument has to be positive integer");
			t=ts.get();
			if(t.kind != ')') error("')' expected");
			return pow_f(x,i);
			}
...
try {
    ...
    cout << "You can use these operators +,-,*,/,(,),sqrt(num), pow(num,num)and %\n";
    cout << "To make a calculation, type your expression and hit ; with enter to get the results.\n";
    cout << "To exit the calculator, press x and enter.\n";
